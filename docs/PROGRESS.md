...
- Fixture hardening: conftest now imports models (metadata populated regardless of collection subset) and disposes the app engine before file removal (Windows file-lock leak). Fixed real `head_sha` OSError crash found by gap tests. Fixed nested `tests/` dir detection found by the real-repo run.
- Shared-types drift check added to CI (`gen_types.py` + `git diff --exit-code`). `.env.example` synced to OpenAI default. README expanded (checks, env table, coverage policy). Mermaid diagram in ARCHITECTURE.md.
